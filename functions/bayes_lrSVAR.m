function EQ=bayes_lrSVAR(data,nlags,var_names,shock_names,irf_lenght,gibbs,const,lr,exog,confianza,parbayes,burnin,verbose)
...
% EQ.P = KxK posterior mean of the long-run identification matrix
% EQ.vardecshock_shockj = K matrices of size Kxirf_lenght, where each row j contains the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Start with OLS estimation of the VAR and also obtain the long-run identification
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Minnesota prior: we set to 1 own variable lags and to zero all other
...
for xx=1:gibbs+burnin  %Start the Gibbs "loop"   
    ...
    if xx>burnin 
        ...
        count=0;
        if demm==1 & demmexo==1
            coeff_temp=coeff_draw(:,2+size(Q.exo,2):end);
        elseif demm==0 & demmexo==0
            coeff_temp=coeff_draw;
        elseif demm==1 & demmexo==0
            coeff_temp=coeff_draw(:,2:end);
        end
        for yy=1:nlags
            somma=coeff_temp(1:K,1+count:K+count)+somma;
            count=count+K;
        end
        QQ=inv(eye(K)-somma)*sigma_draw*(inv(eye(K)-somma))';
        P_temp=(eye(K)-somma)*chol(QQ,'lower');
