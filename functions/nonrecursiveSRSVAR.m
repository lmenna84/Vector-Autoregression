function EQ=nonrecursiveSRSVAR(data,nlags,idmat,var_names,irf_lenght,monte_carlo,const,lr,dum,exog,shock,confianza)
...
% with short run non-recursive restrictions, with optional restrictions on
% variables' interactions
% -----------------------------------
% Inputs:
% data = NxK matrix (N number of observations, K number of variables)
% nlags = desired number of lags (no maximum limit enforced)
% idmat = KxK matrix of zeros and ones specifying contemporaneous restrictions.
% Element (i,j)=0 means structural shock j does not affect variable i 
% contemporaneously; =1 means it does affect it. The matrix must be exactly 
% identified: sum(idmat(:)) must equal K(K+1)/2.
% Optional:
...
% EQ.struct = Kx(N-nlags) matrix containing estimated structural shocks
% ------------------------------------
% The solution method follows Lütkepohl (2005) and Rubio-Ramirez, Waggoner, and Zha (2010)
...
K=size(data,2);
if sum(sum(idmat))~=K*(K+1)/2
    display('The number of restrictions is not correct')
    return
end
Q=reducedformVAR(data,nlags,const,lr,[],dum,exog);
varcovar=Q.sigma;

% Obtain P using the method of Rubio-Ramirez, Waggoner and Zha
L0=varcovar^(1/2);
for j=1:K
    zeri=find(~idmat(:,j));
    numzeri(1,j)=size(zeri,1);
end
[numzeri1,I]=sort(numzeri,'descend');
idmatnew=zeros(K,K);
for j=1:K
    idmatnew(:,j)=idmat(:,I(j));
end
for j=1:K
    Z(:,:,j)=zeros(K,K);
end
...
for j=1:K
    for xx=1:K
        if any(Z(xx,:,j))==1
            eval(['Zbar' num2str(j) '(xx,:)=Z(xx,:,j);']);
        end
    end
end
qtrans=0;
for j=1:K
    if eval(['exist(''Zbar' int2str(j) ''')==0'])
    eval(['Q' int2str(j) 'trans=[qtrans];']);
    else
        if j==1
            Q1trans=Zbar1*L0;
        else eval(['Q' int2str(j) 'trans=[Zbar' int2str(j) '*L0;qtrans];']);
        end
    end
    eval(['[Qtilde,R] = qr(transpose(Q' int2str(j) 'trans));']);
    q(:,j)=Qtilde(:,end);
    qtrans=q';
end
Pnew=L0*q;
...
for jj=1:monte_carlo
    ...
    % Obtain P using the method of Rubio-Ramirez, Waggoner and Zha
        L0_temp=varcovar_temp^(1/2);
        for j=1:K
            if eval(['exist(''Zbar' int2str(j) ''')==0'])
            eval(['Q' int2str(j) 'trans_temp=[qtrans_temp];']);
            else
            if j==1
                Q1trans_temp=Zbar1*L0_temp;
            else eval(['Q' int2str(j) 'trans_temp=[Zbar' int2str(j) '*L0_temp;qtrans_temp];']);
            end
            end
            eval(['[Qtilde_temp,R_temp] = qr(transpose(Q' int2str(j) 'trans_temp));']);
            q_temp(:,j)=Qtilde_temp(:,end);
            qtrans_temp=q_temp';
        end
        Pnew_temp=L0_temp*q_temp;
        P_temp=zeros(K,K);
        for j=1:K
            P_temp(:,I(j))=Pnew_temp(:,j);
        end
        for j=1:K
            if P_temp(j,j)<0
                P_temp(:,j)=-P_temp(:,j);
