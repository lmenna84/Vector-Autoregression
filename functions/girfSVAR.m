function EQ=girfSVAR(data,nlags,var_names,shock_names,irf_lenght,monte_carlo,const,lr,dum,exog,confianza,verbose)
...
% with generalized impulse response functions (Pesaran and Shin, 1998).
% GIRFs are order-invariant (unlike Cholesky) and variance-normalized.
% -----------------------------------
% Inputs:
...
%                  95% confidence line, in the second row the GIRF, and in the third row the
%                  5% confidence line. i is the variable, j is the shock.
% EQ.P = KxK variance-covariance matrix of reduced-form residuals
...
% EQ.stdirf = Kxirf_lenghtxK array containing standard deviations of GIRFs from Monte Carlo
% EQ.struc = KxT matrix of generalized structural shocks (non-orthogonal)
% EQ.irf_sim = Kxirf_lenghtxmonte_carloxK array of all GIRF draws from Monte Carlo
% ------------------------------------
% The solution method follows Lütkepohl (2005) and Pesaran and Shin (1998)
...
for jj=1:monte_carlo
    ...
    for xx=1:K
        ...
        W_temp(1:K,1)=P_temp(xx,xx)^(-1/2)*P_temp*w_temp;
        for yy=1:irf_lenght
...
% In the generalized approach, the structural shocks are not truly
% "structural" as the identifying matrix does not generate orthogonal
% shocks. They are variance-normalized innovations.
for ii=1:K
    temp=inv(P(ii,ii)^(-1/2)*P)*Q.resid';
    struc(ii,:)=temp(ii,:);
end
EQ.struc=struc;
