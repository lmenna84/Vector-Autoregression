function EQ=nonrecursiveLRSVAR(data,nlags,idmat,var_names,shock_names,irf_lenght,monte_carlo,const,lr,dum,exog,shock,confianza)
...
% with long run non-recursive zero restrictions, with optional restrictions on
...
% allowing non-recursive (non-triangular) patterns.
% Optional:
...
% EQ.P = KxK estimated impact matrix satisfying long-run restrictions
...
% Recover impact matrix from long-run multiplier
P=(eye(K)-somma)*P_lr;
EQ.P=P;
...
for jj=1:monte_carlo
    ...
    qtrans_temp=0;
        ...
        P_lr_temp=zeros(K,K);
        for j=1:K
            P_lr_temp(:,I(j))=Pnew_temp(:,j);
        end
        for j=1:K
            if P_lr_temp(j,j)<0
                P_lr_temp(:,j)=-P_lr_temp(:,j);
            end
        end
        P_temp=(eye(K)-somma_temp)*P_lr_temp;
