%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% EXAMPLES FOR ivSVAR_blockbootstrap
%% Author: Lorenzo Menna
%% This script demonstrates all features of the ivSVAR_blockbootstrap function
%% (Proxy-SVAR with external instruments, block bootstrap confidence intervals)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
EQ1 = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, shock_names, 40, 1000, true);
close all;
...
rng(12345);
EQ_abs = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
                                shock_names, 40, 1000, true);
rng(12345);
EQ_rel = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
...
rng(12345);
EQ_unit = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
                                 shock_names, 40, 1000, true);
rng(12345);
EQ_10   = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
...
%% ========================================================================
disp('========================================');
disp('EXAMPLE 5: Block bootstrap with different confidence levels');
disp('========================================');

EQ_90 = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
                               shock_names, 40, 1000, true, [], [], [], [], 5);    % 90% CI
EQ_68 = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
...
EQ_seas = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
                                 shock_names, 40, 1000, true, [], 'quarter');
close all;

disp('Quarterly seasonal dummies included in reduced-form VAR');
disp('Block bootstrap applied to seasonally-adjusted residuals');
disp('Dummy coefficients accessible via the reduced-form VAR estimation');
fprintf('\n\n');
...
EQ_exog = ivSVAR_blockbootstrap(data_crisis_inst, data_crisis_other, iv, nlags, ...
...
EQ_lr = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
...
sgtitle('IRFs to Oil Supply Shock (90% Block Bootstrap CI)');
...
EQ_verb = ivSVAR_blockbootstrap(data_inst, data_other, iv, nlags, var_names, ...
...
%% ========================================================================
%% EXAMPLE 13: When to use block bootstrap
%% ========================================================================
disp('========================================');
disp('EXAMPLE 13: When to use block bootstrap for proxy-SVARs');
disp('========================================');

disp('Block bootstrap is appropriate when:');
disp('  1. Data exhibits serial correlation (typical in macroeconomic time series)');
disp('     - Consecutive residuals are resampled in blocks');
disp('     - Autocorrelation structure is preserved within each block');
disp('  ');
disp('  2. Residuals are approximately homoskedastic over time');
disp('     - Block bootstrap does not explicitly model heteroskedasticity');
disp('     - If heteroskedasticity is a concern, use ivSVAR_wildbootstrap instead');
disp('  ');
disp('  3. Sample size is reasonably large (T >= 100)');
disp('     - Optimal block length is computed automatically as l = ceil(T^(1/4))');

l_opt = ceil((T - nlags)^(1/4));
fprintf('\n  Optimal block length for this dataset (T=%d, T_eff=%d): l = %d\n', ...
        T, T-nlags, l_opt);
fprintf('  Number of blocks drawn per bootstrap repetition: ~%d\n', ...
        ceil((T-nlags)/l_opt));
fprintf('\n');

disp('Block bootstrap reference: Jentsch & Lunsford (2019, RESTAT)');
...
disp('========================================');
disp('Key proxy-SVAR (block bootstrap) features demonstrated:');
...
disp('- When to use block bootstrap');
disp(' ');
disp('Key concepts:');
...
disp('- Block bootstrap preserves autocorrelation in confidence intervals');
